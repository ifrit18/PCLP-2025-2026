...
def check_readme(config):
    ...
    if not readme.is_file():
        ...
        readme_points = 0 # for now
        log('Found non-empty\'{}\'!'.format(
            readme.name))

    update_grade(config, readme_points)
    log('----- running check_readme - NO grade given yet, final points are given after manual grading.')
    return 0
...
def check_style(config):
    ...
    if child.returncode != 0 or len(extract_stdout(child)) != 0:
        ...
        if coding_style_grade < config.coding_style:
            reason = 'No coding style errors automatically detected (tests points {}/{}). Final points are given after manual grading.'.format(config.grade, config.tests_points)
    ...
    # TODO: Uncomment this when we have a sound file for future assignments.
...
def run_test(task, test, use_valgrind=False, ):
    ...
    try:
        child = subprocess.run(cmd, shell=False if not use_valgrind else True,
                               ...
                               timeout=task.timeout if not use_valgrind else task.timeout_valgrind,
...
def checker_run():
    ...
    log('\nTotal: {:g}/100 (README points will be given after manual grading)'.format(config.grade))
