...
usage: check [-h] [--task {fizzbuzz}] [--legend {True,False}]
...
rula checker-ul pentru problema `fizzbuzz`:

```shell
$ python3 ./check --task fizzbuzz
+++++ check_deps ...
system               - Linux
...
+++++ running all tasks
	...
	+++++ running task fizzbuzz (80p) ...

		+++++ running test 00-fizzbuzz
			running test       - OK
				stage time: 0:00:00.459439
			running check      - OK
				stage time: 0:00:00.059390
		----- running test 00-fizzbuzz - grade 8/8

		+++++ running test 01-fizzbuzz
			running test       - OK
				stage time: 0:00:00.005056
			running check      - OK
				stage time: 0:00:00.044647
		----- running test 01-fizzbuzz - grade 8/8

		+++++ running test 02-fizzbuzz
			running test       - OK
				stage time: 0:00:00.005140
			running check      - OK
				stage time: 0:00:00.039993
		----- running test 02-fizzbuzz - grade 8/8

		+++++ running test 03-fizzbuzz
			running test       - OK
				stage time: 0:00:00.005098
			running check      - OK
				stage time: 0:00:00.040885
		----- running test 03-fizzbuzz - grade 8/8

		+++++ running test 04-fizzbuzz
			running test       - OK
				stage time: 0:00:00.005221
			running check      - OK
				stage time: 0:00:00.037343
		----- running test 04-fizzbuzz - grade 8/8

		+++++ running test 05-fizzbuzz
			running test       - OK
				stage time: 0:00:00.005116
			running check      - OK
				stage time: 0:00:00.038376
		----- running test 05-fizzbuzz - grade 8/8

		+++++ running test 06-fizzbuzz
			running test       - OK
				stage time: 0:00:00.005348
			running check      - OK
				stage time: 0:00:00.037244
		----- running test 06-fizzbuzz - grade 8/8

		+++++ running test 07-fizzbuzz
			running test       - OK
				stage time: 0:00:00.005175
			running check      - OK
				stage time: 0:00:00.035926
		----- running test 07-fizzbuzz - grade 8/8

		+++++ running test 08-fizzbuzz
			running test       - OK
				stage time: 0:00:00.005103
			running check      - OK
				stage time: 0:00:00.036047
		----- running test 08-fizzbuzz - grade 8/8

		+++++ running test 09-fizzbuzz
			running test       - OK
				stage time: 0:00:00.005169
			running check      - OK
				stage time: 0:00:00.035196
		----- running test 09-fizzbuzz - grade 8/8

	----- running task fizzbuzz - grade 80/80
...
```bash
$ make pack
zip -FS "PCLP3XYCA_FirstnameLastname_Tema0.zip".zip *.c *.h Makefile README
  adding: fizzbuzz.c (deflated 40%)
...
De exemplu, pentru problema `fizzbuzz`, testele se găsesc în `tasks/fizzbuzz/tests `:

```shell
$ ls -l tasks/fizzbuzz/tests
total 0
drwxr-xr-x  7 dariusn  staff  224 Oct 31 08:12 00-fizzbuzz
drwxr-xr-x  5 dariusn  staff  160 Oct 31 08:12 01-fizzbuzz
...
```

Dacă dorim să inspectăm ce output / eroare a produs programul nostru la problema `fizzbuzz`, pe testul `00`, putem să ne uităm la următoarele fișiere:

```shell
$ ls -l tasks/fizzbuzz/tests/00-fizzbuzz
total 40
-rw-r--r--  1 dariusn  staff   63 Oct 31 08:12 00-fizzbuzz.in    # Datele de intrare (STDIN) pentru testul 00.
-rw-r--r--  1 dariusn  staff  243 Oct 30 22:34 00-fizzbuzz.ref   # Datele de ieșire corecte pentru testul 00.
-rw-r--r--  1 dariusn  staff  243 Oct 31 07:48 00-fizzbuzz.out   # Datele de ieșire (STDOUT) produse de programul nostru pentru testul 00 (sau chiar un fișier, în funcție de temă).
-rw-r--r--  1 dariusn  staff   12 Oct 31 07:48 00-fizzbuzz.error # Datele de eroare (STDERR) produse de programul nostru pentru testul 00.
```
...
$ ./fizzbuzz < tasks/fizzbuzz/tests/00-fizzbuzz/00-fizzbuzz.in

# Rulare manuală test 00: citire input cu redirectare din fișier, afișare cu redirectare output în fișierul hai_steaua.out
$ ./fizzbuzz < tasks/fizzbuzz/tests/00-fizzbuzz/00-fizzbuzz.in > hai_steaua.out
```
